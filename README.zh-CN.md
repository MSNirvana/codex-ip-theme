...
Codex IP Theme 可以自动处理简单白底/纯色背景、生成透明 PNG、创建跨平台启动器，并通过仅监听本机回环地址的 CDP 注入主题。它**不会**修改 `app.asar`、官方应用包、代码签名、登录状态、API Key 或模型供应商配置。
...
必填内容只有图片。可选内容包括主题名称、强调色、背景色、侧栏色、文字色、裁切范围、角色位置和输出目录。
...
```text
使用 $codex-ip-theme，把我已经生成的主题中输入框角色替换为这张图片。
```

注入器运行时会监听图片、`theme/config.json` 和 `theme/theme.css`。文件变化后约两秒内重新应用，无需修改或重新打包 Codex。
